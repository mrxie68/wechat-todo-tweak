    ...
    void (^send)(void) = ^{
        ...
        if ([mgr respondsToSelector:@selector(SendTextMessage:toUsrName:)]) {
            ...
            if (wrapCls) {
                ...
                // 待办回复以“待办联系人”身份本地插入（左侧气泡），避免自发自回
                BOOL isTodo = [chatId isEqualToString:kAITodoChatId];
                NSString *fromUsr = isTodo ? kAITodoChatId : selfUsr;
                NSString *toUsr = isTodo ? selfUsr : chatId;
                if (fromUsr.length > 0) {
                    wrap = [[wrapCls alloc] initWithMsgType:1 nsFromUsr:fromUsr];
                } else {
